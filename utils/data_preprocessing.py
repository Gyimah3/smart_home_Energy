...
def load_and_preprocess_data(file_path):
    try:
        ...
        logger.info("Identifying and processing features...")
        numerical_cols = [
        ...
        categorical_cols = [
            ...
            'weather_condition', 'is_weekend', 'season'
        ]
        ...
        energy_target = df['energy_kwh'].values.astype(np.float32)
        user_target = df['user_id'].values.astype(np.int64)
        anomaly_target = df['anomaly_score'].values.astype(np.float32)
        ...
        X_train, X_test, y_energy_train, y_energy_test, y_user_train, y_user_test, y_anomaly_train, y_anomaly_test = train_test_split(
            features, energy_target, user_target, anomaly_target,
            test_size=0.2, random_state=42
        )
        
        logger.info("Data preprocessing completed successfully!")
        return X_train, X_test, y_energy_train, y_energy_test, y_user_train, y_user_test, y_anomaly_train, y_anomaly_test
...
def create_sequences(features, energy_target, user_target, anomaly_target, seq_length):
    try:
        ...
        for i in range(len(features) - seq_length + 1):
            ...
            y_energy.append(energy_target[i+seq_length-1])
            y_user.append(user_target[i+seq_length-1])
            y_anomaly.append(anomaly_target[i+seq_length-1])
        
        logger.info(f"Sequence shape: {np.array(X).shape}")
        return (
